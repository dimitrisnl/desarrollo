...
type ProviderEmailServer = {
  host: string;
  port: number;
  auth: ProviderEmailAuth;
};

type ProviderEmailAuth = {
  user: string;
  pass: string;
};

type ProviderEmailOptions = {
  name?: string;
...
export const sendMagicLink = ({
  ...
  baseUrl,
  provider,
}: SendMagicLinkProps): Promise<void> => {
  return new Promise((resolve, reject) => {
    const {server, from} = provider;
    const site = baseUrl.replace(/^https?:\/\//, '');

    return nodemailer.createTransport(server).sendMail(
      {
        to: email,
        from,
        ...
        headers: {
          'X-PM-Message-Stream': 'magic-links',
        },
      },
      (error) => {
        if (error) {
